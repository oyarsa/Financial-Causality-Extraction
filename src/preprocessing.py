...
def load_examples(
    ...
    Tuple[TensorDataset, List[FinCausalExample], List[FinCausalFeatures]], TensorDataset
]:
...
class FinCausalProcessor:
    def get_examples(self, file_path: Path) -> List[FinCausalExample]:
        input_data = pd.read_csv(
            file_path, index_col=0, delimiter=";", header=0, skipinitialspace=True
        )
...
def _check_is_max_context(
    doc_spans: Union[List[dict], List[UserDict]], cur_span_index: int, position: int
) -> bool:
...
def fincausal_convert_example_to_features(
    example: FinCausalExample, max_seq_length: int, doc_stride: int, is_training: bool
) -> List[FinCausalFeatures]:
    features = []
    if is_training:
        ...
        actual_cause_text = " ".join(
            example.doc_tokens[start_cause_position : (end_cause_position + 1)]
        )
        cleaned_cause_text = " ".join(
            whitespace_tokenize(_run_split_on_punc(example.cause_text))
        )
        if actual_cause_text.find(cleaned_cause_text) == -1:
            logger.warning(
                "Could not find cause: '%s' vs. '%s'",
                actual_cause_text,
                cleaned_cause_text,
            )
            return []
        ...
        actual_effect_text = " ".join(
            example.doc_tokens[start_effect_position : (end_effect_position + 1)]
        )
        cleaned_effect_text = " ".join(
            whitespace_tokenize(_run_split_on_punc(example.effect_text))
        )
        if actual_effect_text.find(cleaned_effect_text) == -1:
            logger.warning(
                "Could not find effect: '%s' vs. '%s'",
                actual_effect_text,
                cleaned_effect_text,
            )
            return []
    ...
    if is_training:
        ...
        if example.end_cause_position < len(example.doc_tokens) - 1:
            tok_cause_end_position = (
                orig_to_tok_index[example.end_cause_position + 1] - 1
            )
        else:
        ...
        if example.end_effect_position < len(example.doc_tokens) - 1:
            tok_effect_end_position = (
                orig_to_tok_index[example.end_effect_position + 1] - 1
            )
        else:
    ...
    while len(spans) * doc_stride < len(all_doc_tokens):
        encoded_dict: BatchEncoding = tokenizer.encode_plus(
            span_doc_tokens,
    ...
    for span in spans:
        ...
        p_mask[
            np.where(np.array(span.data["input_ids"]) == tokenizer.sep_token_id)[0]
        ] = 1
        ...
        if is_training:
            ...
            if not (
                tok_cause_start_position >= doc_start
            ...
            if out_of_span:
                ...
                effect_start_position = (
                    tok_effect_start_position - doc_start + doc_offset
                )
...
def fincausal_convert_examples_to_features(
    examples: List[FinCausalExample],
    tokenizer: Union[PreTrainedTokenizer, PreTrainedTokenizerFast],
    max_seq_length: int,
    doc_stride: int,
    is_training: bool,
    ...
    threads: int = 1,
) -> Union[List[FinCausalFeatures], Tuple[List[FinCausalFeatures], TensorDataset]]:
    ...
    if return_dataset == "pt":
        ...
        if not is_training:
            ...
            all_cause_start_positions = torch.tensor(
                [f.cause_start_position for f in features], dtype=torch.long
            )
            all_cause_end_positions = torch.tensor(
                [f.cause_end_position for f in features], dtype=torch.long
            )
            all_effect_start_positions = torch.tensor(
                [f.effect_start_position for f in features], dtype=torch.long
            )
            all_effect_end_positions = torch.tensor(
                [f.effect_end_position for f in features], dtype=torch.long
            )
            dataset = TensorDataset(
                all_input_ids,
                all_attention_masks,
